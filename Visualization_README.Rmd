...
# --- DEV to remove on package integration --- #
library(devtools)
# Load scripts
devtools::load_all()
# ------------------#
...
dataframe.datalist <- kp$find_data('mapped_results', default_threshold=0.01) 
fig <- make.graph(dataframe.datalist, 'Spermatogonia', 0.01, debug=TRUE) # These are the minimum required parameters
fig
```
...
```{r}
# Obtain a list of edgelists from the dataframe list; specifiy node of interest
edgelist.datalist <- kp$get_edgelists(dataframe.datalist, 'g__Turicibacter')
```
...
fig <- make.graph(dataframe.datalist, 'g__Turicibacter', 0.01, debug=TRUE)